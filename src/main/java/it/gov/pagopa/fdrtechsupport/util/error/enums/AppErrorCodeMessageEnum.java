...
public enum AppErrorCodeMessageEnum {
    ...
    ERROR("0500", "system.error", RestResponse.Status.INTERNAL_SERVER_ERROR, "An error occurred during computation. This could be caused by an applicative error and it is probably required to open an issue."),
    BAD_REQUEST("0400", "bad.request", RestResponse.Status.BAD_REQUEST, "A generic 'Bad Request' error is occurred during request validation"),
    BAD_REQUEST_INPUT_JSON("0401", "bad.request.inputJson", RestResponse.Status.BAD_REQUEST, "A generic error occurred during execution of request syntactic validation"),
    BAD_REQUEST_INPUT_JSON_INSTANT(
            "0402", "bad.request.inputJson.instant", RestResponse.Status.BAD_REQUEST, "An error occurred during execution of request syntactic validation, in particular regarding the analysis of date values"),
    BAD_REQUEST_INPUT_JSON_ENUM(
            "0403", "bad.request.inputJson.enum", RestResponse.Status.BAD_REQUEST, "An error occurred during execution of request syntactic validation, in particular regarding the analysis of enumerative values"),
    ...
    AppErrorCodeMessageEnum(
            String errorCode, String errorMessageKey, RestResponse.Status httpStatus, String openAPIDescription
    ) {
